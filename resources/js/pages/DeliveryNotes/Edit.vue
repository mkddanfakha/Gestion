<template>
  <AppLayout>
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
      <div>
        <h1 class="h2 mb-1">Modifier le bon de livraison</h1>
        <p class="text-muted mb-0">{{ deliveryNote.delivery_number }}</p>
      </div>
          ...
          <div class="card sticky-top" style="top: 20px;">
            ...
            <div class="card-body">
              ...
              <div class="alert alert-warning">
                ...
                Attention : Modifier ce bon de livraison après validation ne modifiera pas le stock. Seul un nouveau BL peut ajuster le stock.
              </div>
              ...
              <div class="d-grid gap-2">
                ...
                >
                  ...
                  {{ processing ? 'Modification...' : 'Modifier le bon de livraison' }}
                </button>
                <Link
                  :href="route('delivery-notes.index')"
                  class="btn btn-outline-secondary"
...
const form = useForm({
  supplier_id: props.deliveryNote.supplier_id,
  purchase_order_id: props.deliveryNote.purchase_order_id,
  delivery_date: formatDateForInput(props.deliveryNote.delivery_date),
  status: props.deliveryNote.status,
  notes: props.deliveryNote.notes || '',
  invoice_number: props.deliveryNote.invoice_number || '',
  tax_amount: Number(props.deliveryNote.tax_amount) || 0,
  discount_amount: Number(props.deliveryNote.discount_amount) || 0,
  subtotal: Number(props.deliveryNote.subtotal) || 0,
  total_amount: Number(props.deliveryNote.total_amount) || 0,
...
// Initialiser le bon de commande sélectionné au chargement
onMounted(() => {
  // Trouver le BC du BL
  const po = props.purchaseOrders.find(po => po.id === props.deliveryNote.purchase_order_id)
  if (po) {
    selectedPurchaseOrder.value = po
    poSearchQuery.value = po.po_number
  }
